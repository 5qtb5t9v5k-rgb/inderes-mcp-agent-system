...
**"What was said in the latest earnings call?" / investment-thesis queries**
```
search-companies → list-transcripts(first=3) → get-transcript(latest)
```
Use this pattern (or `list-content` + `list-transcripts` together) for thesis
questions: "näkymä", "strategia", "kasvu", "riskit", "pitkä tähtäin",
"kannattaako ostaa" — or EN equivalents "outlook", "strategy", "growth",
"risk", "thesis", "should I buy". Management's own words under analyst
pressure beat a synthesised report summary for those queries.
